...
class AutomatoMaquinaVendas:
    def __init__(automato):
        ...
        # inicia com estado inicial, saldo = 0
        automato.estado_atual = 's0'
        ...
        automato.transicoes = {
            ...
            # Ao acionar botao, deve ter 2 reias no saldo, ou nao ira retornar nada 'n'
            ('s8', 'b'): ('s0', 'r'),  # Libera refrigerante
    ...
    def entrada(automato, moeda_ou_botao):
        if (automato.estado_atual, moeda_ou_botao) in automato.transicoes:
            proximo_estado, saida = automato.transicoes[(automato.estado_atual, moeda_ou_botao)]
            automato.estado_atual = proximo_estado
            return saida
        else:
...
def alterar_saldo(coin_value):
    ...
    saida = vm.entrada(coin_value)
    saldo = vm.estados[vm.estado_atual]
    texto_saldo["text"] = "Saldo: R$ " + str(saldo)
    if saida.startswith('t'):
        if saida == 't25':
...
botao_liberar_refri00 = Button(janela, text="R$ 1.00", command=lambda: alterar_saldo('m100'), bg="gold2", fg="cornsilk1", width=10, height=1 , font=("Calibri", 12))
botao_liberar_refri00.grid(row=2, column=3, padx=10, pady=10)
...
botao_liberar_refri = Button(janela, text="Liberar Refrigerante", command=liberar_refrigerante,  font=("Calibri", 14), bg='darkolivegreen', fg='cornsilk')
botao_liberar_refri.grid(row=3, columnspan=5, pady=30)
